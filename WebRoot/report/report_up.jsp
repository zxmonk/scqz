...
<div id="main">
	...
	<p class="tag2">
		*请注意在选择日期时，请选择报告内容日期，而不是报告提交日期 <br />*上传文件格式为word2003文档(.doc)
	</p>

	<%
		String result2 = null;
		result2 = (String) request.getSession().getAttribute("result2");
		if (result2 == null) {

		} else {
			out.println("<font color=\"red\">上传成功！</font><br/>");
		}
		request.getSession().removeAttribute("result2");
	%>


	<form action="/scqz/report/ReportUp" method="post"
		...
		<label>报告类型（必填）</label> <select name="reporttype">
			...
			<option value="51164_攀枝花地龙井">51164攀枝花防震减灾局</option>
		...
		%>
		<label>选择时间（必填）</label> <input type=text name="date"
			onClick="WdatePicker()"
			value="<%out.print(formatter.format(new Date()));%>" /> <font
			color="red">（例：2014年6月的月报 选择2014年6月的任意一天均可,年报同理） </font> <br /> <label>选择文件（必填）</label>
		<input type="file" name="file" id="fileUpload" /> <font color="red">（支持拖拽）
		</font> <br /> <label>上传人员（必填）</label> <input type=text name="upname"
			value="" id="nameUpload" /> <br /> <input type="submit" value="提交" />
